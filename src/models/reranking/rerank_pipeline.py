from .rerank import reranking_component, encode_features
import logging
import click
from ...features.build_features import load_pickle
from typing import Optional, List

def recommendation_reranking_component(uris_dict_filepath: str, desired_distribution_filepath: str, base_recommendations: List[str], n_recommendations: Optional[int]=50):
    uris_dict = load_pickle(uris_dict_filepath)
    desired_distribution = load_pickle(desired_distribution_filepath)
    features = encode_features(base_recommendations, uris_dict)
    reranked = reranking_component(base_recommendations, features, n_recommendations, desired_distribution)
    return reranked
...
def main(uris_dict_filepath: str, desired_distribution_filepath: str):
    # uris_dict = load_pickle(uris_dict_filepath)
    # desired_distribution = load_pickle(desired_distribution_filepath)
    ...
    reranked_rec = recommendation_reranking_component(uris_dict_filepath, desired_distribution_filepath, test_rec, 100)
    logging.info(f"Reranked: {reranked_rec}")
