...
from .GNN_models import GraphSAGE, MLPPredictor, DotPredictor, compute_loss, compute_auc, train, predict
...
def recommend_n_diverse(features: NDArray, playlists: NDArray, user_playlist: NDArray, n: Optional[int]=10, sample_size: Optional[int]=5, hid_dim: Optional[int]=16, epochs: Optional[int]= 100):
    ...
    pos_graph = dgl.graph((pos_edges[:, 0].flatten(), pos_edges[:, 1].flatten()), num_nodes=features.shape[0])
    pos_graph.ndata['feat']=features
    neg_graph = dgl.graph((neg_edges[:, 0].flatten(), neg_edges[:, 1].flatten()), num_nodes=features.shape[0])
    neg_graph.ndata['feat']=features
    total_start_edges = np.concatenate((pos_edges[:, 0], neg_edges[:, 0]), axis=0).flatten()
    total_end_edges = np.concatenate((pos_edges[:, 1], neg_edges[:, 1]), axis=0).flatten()
    graph = dgl.graph((total_start_edges, total_end_edges), num_nodes=features.shape[0])
    graph.ndata['feat']=features

    model = GraphSAGE(graph.ndata['feat'].shape[1], hid_dim)
    predictor = DotPredictor()
    model, predictor, hidden_dim = train(model, predictor, graph, pos_graph, neg_graph, epochs=20, verbose=True)
    candidates = generate_candidate_edges(pos_edges, neg_edges, user_playlist)
    print(candidates.shape)
    predictions = predict(candidates, features, predictor, hidden_dim)
    print(predictions)
...
def main(input_filepath: str, model_filepath: str, output_filepath: str):
    features = pd.read_csv(os.path.join(input_filepath, "features.csv"), index_col=False).to_numpy()[:, 2:13]
    playlists = pd.read_csv(os.path.join(input_filepath, "playlists.csv")).to_numpy()
    user_playlist = pd.read_csv(os.path.join(input_filepath, "user_playlist.csv")).to_numpy()[:, 1]
    SAMPLE_SIZE = 3
    recommend_n_diverse(features, playlists, user_playlist, 100, 10, 32, epochs=20)
